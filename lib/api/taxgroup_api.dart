...
class TaxGroupApi {
  ...
  Future<List<Map<String, dynamic>>> _fetchRows(
    ...
    if (apiUrl.trim().isEmpty) {
      throw Exception('Tax Group API URL is not configured');
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(
        'Tax Group API failed with status ${response.statusCode}: $apiUrl',
      );
    }

    return TaxGroupPayload.rowsFromJson(jsonDecode(response.body));
  }

  Future<TaxGroupSyncResult> saveTaxGroupRows(
    ...
    return db.transaction((txn) async {
      if (clearExistingData) {
        await txn.delete('Tax_Group');
      }

      final savedCount = await _upsertRows(txn, rows);

      return TaxGroupSyncResult(savedCount: savedCount);
    });
  }
  ...
  Future<int> _upsertRows(
    ...
    for (final row in rows) {
      ...
      final taxGroupCode = normalizedRow['Tax_Group_Code'];

      if ((taxCode == null || taxCode.toString().trim().isEmpty) &&
          (taxGroupCode == null || taxGroupCode.toString().trim().isEmpty)) {
        continue;
      }
  ...
  Map<String, dynamic> _normalizeRow(Map<String, dynamic> row) {
    ...
    for (final column in TaxGroupPayload.columns) {
...
class TaxGroupPayload {
  ...
  static bool _looksLikeTaxGroupRow(Map<String, dynamic> row) {
    return _readValue(row, 'Tax_Code') != null ||
        _readValue(row, 'Tax_Group_Code') != null ||
        _readValue(row, 'Tax_Group_Name') != null;
  }
...
class TaxGroupSyncResult {
  const TaxGroupSyncResult({required this.savedCount});
