...
class MenuItemApi {
  ...
  static const String menuItemApiUrl =
      'https://fudo.magnexsolutions.com/api/v1_5/menuitem/940T0003/002';

  final http.Client _client;

  Future<MenuItemSyncResult> syncMenuItems({
    String apiUrl = MenuItemApi.menuItemApiUrl,
    Map<String, String>? headers,
    bool clearExistingData = false,
  }) async {
    final rows = await _fetchRows(apiUrl, headers: headers);

    return saveMenuItemRows(
      rows,
  ...
  Future<List<Map<String, dynamic>>> _fetchRows(
    ...
    if (apiUrl.trim().isEmpty) {
      throw Exception('Menu Item API URL is not configured');
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(
        'Menu Item API failed with status ${response.statusCode}: $apiUrl',
      );
    }

    return MenuItemPayload.rowsFromJson(jsonDecode(response.body));
  }

  Future<MenuItemSyncResult> saveMenuItemRows(
    ...
    return db.transaction((txn) async {
      ...
      return MenuItemSyncResult(savedCount: savedCount);
    });
  }
  ...
  Map<String, dynamic> _normalizeRow(Map<String, dynamic> row) {
    ...
    for (final column in MenuItemPayload.columns) {
...
class MenuItemPayload {
  ...
  static bool _looksLikeMenuItemRow(Map<String, dynamic> row) {
    return _readValue(row, 'Menu_ID') != null ||
        _readValue(row, 'Menu_Name') != null;
  }
...
class MenuItemSyncResult {
  const MenuItemSyncResult({required this.savedCount});
